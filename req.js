...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const approvedList = document.querySelector('#approvedList tbody');
  const onholdList = document.querySelector('#onholdList tbody');
  ...
  form.addEventListener('submit', (event) => {
    ...
    addRequestToTable(formData, requestList);
  ...
  function loadRecentRequests() {
    ...
    requests.forEach((request) => addRequestToTable(request, requestList));
  }
  ...
  // Add request to the table (either recent, approved, or on hold)
  function addRequestToTable(request, table) {
  ...
  // Move a request to a different list (approved or on hold)
  function moveRequest(request, targetKey, row) {
    const requests = getFromLocalStorage('bloodRequests');
    const updatedRequests = requests.filter((r) => r.date !== request.date);

    // Save the updated pending requests and add the request to the target list (approved or on hold)
    saveToLocalStorage('bloodRequests', updatedRequests);
    saveToLocalStorage(targetKey, request);

    // Remove the request from the "Recent Requests" table and move it to the appropriate list
    row.remove();
    if (targetKey === 'approvedRequests') {
      addRequestToTable(request, approvedList);
      alert('Request moved to Approved Requests.');
    } else {
      addRequestToTable(request, onholdList);
      alert('Request moved to On Hold.');
    }
  }
